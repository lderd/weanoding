...
public class UserController {
	...
	public ResponseEntity<? extends BaseResponseBody> delete(
			@PathVariable("userId") @ApiParam(value="탈퇴할 회원 아이디", required = true) String userId) {
	...
	@PatchMapping("/{userId}")
	@ApiOperation(value = "회원정보 수정", notes = "<strong>아이디</strong>를 통해 회원정보를 수정 한다.")
	@ApiResponses({
			@ApiResponse(code = 200, message = "성공"),
			@ApiResponse(code = 401, message = "인증 실패"),
			@ApiResponse(code = 404, message = "사용자 없음"),
	...
	public ResponseEntity<? extends BaseResponseBody> update(
			@PathVariable("userId") @ApiParam(value="수정할 회원 아이디") String userId,
			@RequestBody @ApiParam(value="수정할 정보", required = true) UserUpdatePostReq updateInfo) {
	...
	@GetMapping("/{userId}")
	@ApiOperation(value = "아이디 중복 체크", notes = "<strong>아이디</strong>를 통해 중복 체크 한다.")
	@ApiResponses({
			@ApiResponse(code = 200, message = "성공"),
			@ApiResponse(code = 401, message = "인증 실패"),
